...
import { isYouthAdmin } from '@/lib/auth/roleAccess';

export default function SKYouthAdminLayout({
  children,
}: {
  ...
  if (!isYouthAdmin(auth.user)) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md text-center">
          <p className="text-red-600 font-bold text-lg">Access Denied</p>
          <p className="text-gray-600 mt-2">SK-Youth Admin access is required to view this page.</p>
  ...
  const youthAdminLinks = [
    { label: 'Dashboard', href: '/admin/sk-youth', show: true, icon: '📊' },
    { label: 'Youth Management', href: '/admin/sk-youth/management', show: true, icon: '👦' },
    { label: 'Programs', href: '/admin/sk-youth/programs', show: true, icon: '🎓' },
    { label: 'Reports', href: '/admin/sk-youth/reports', show: true, icon: '📋' },
  ];
        ...
        <div className="p-6 border-b border-gray-200">
          {sidebarOpen && (
            <>
              ...
              <p className="text-xs text-gray-500 mt-1">SK-Youth Admin</p>
            </>
          )}
        </div>

        <nav className="flex-1 py-4 space-y-2 px-3">
          {youthAdminLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
